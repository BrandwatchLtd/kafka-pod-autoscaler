...
public class StatefulSetResourceFactory implements ScaledResourceFactory {
    @Override
    public boolean supports(KubernetesClient client, String namespace, ScaleTargetRef scaleTargetRef) {
        return scaleTargetRef.getKind().equalsIgnoreCase("statefulset");
    }

    @Override
    public ScaledResource create(KubernetesClient client, String namespace, ScaleTargetRef scaleTargetRef) {
        var resource = client.apps().statefulSets().inNamespace(namespace).withName(scaleTargetRef.getName());
